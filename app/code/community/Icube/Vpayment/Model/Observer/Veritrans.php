...
class Icube_Vpayment_Model_Observer_Veritrans
{
    public function captureVT(Varien_Event_Observer $observer) {
        ...
        if($payment_method === 'vpayment') {
            ...
            try {
                $sql = "SELECT transaction_id,status FROM veritrans where order_id='".$order_id."'";
...
//                    }
                        ...
                        switch ($codeResp) {
                            case '200':
                                $sql = "UPDATE veritrans SET status = 'captured' WHERE transaction_id = '".$transaction_id."'";
...
//            Mage::log("transaction_id:".$transaction_id, null, 'transaction_id.log', true);
            ...
            if($result->status_code == '200') {
                ...
                $sql = "UPDATE veritrans SET status = 'cancel' WHERE transaction_id = '".$transaction_id."'";
                $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
                $connection->query($sql);
            }
        }
